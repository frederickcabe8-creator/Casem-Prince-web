...
class DashboardController extends Controller
{
    ...
    {
        try {
            // Overview stats with fallbacks
            $totalRevenue = Order::where('payment_status', 'paid')->sum('total_amount') ?? 0;
            $totalOrders = Order::count();
            $totalCustomers = User::where('role', 'customer')->count() ?: User::count();
            ...
            // Top products - SAFE VERSION
            $topProducts = collect();
            if (DB::getTableBuilder()->getConnection()->getSchemaBuilder()->hasTable('order_items')) {
            ...
            return view('admin.dashboard', [
                ...
                'ordersThisMonth' => 0, 'last30Days' => collect(), 'ordersByStatus' => collect(),
                'topProducts' => collect(), 'recentOrders' => collect(), 'monthlyRevenue' => collect(), 'paymentMethods' => collect()
            ]);
        }
    }
}